...
class SqlParser {
    ...
    var $manager_language, $modx_charset;

	function SqlParser($host, $user, $password, $db, $prefix='modx_', $adminname, $adminemail, $adminpass, $connection_charset= 'utf8', $manager_language, $modx_charset) {
		$this->host = $host;
		$this->dbname = $db;
		$this->prefix = $prefix;
		$this->user = $user;
		...
		$this->manager_language = $manager_language;
		$this->modx_charset = $modx_charset;
	...
	function process($filename) {
		...
		$idata = str_replace('{MANAGERLANGUAGE}', $this->manager_language, $idata);
		$idata = str_replace('{MODXCHARSET}', $this->modx_charset, $idata);
