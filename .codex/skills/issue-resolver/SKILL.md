...
4. 修正方針を先に判定し、軽微な修正はそのまま `implement-fix` へ進む。
5. 問題が分かりにくい場合、影響範囲が読みにくい場合、または実装が複雑な場合は `.agent/PLANS.md` に従って ExecPlan を作成する。
6. 不具合調査では手動確認より先に `php evo` で設定・DB状態・キャッシュ状態を確認する。
...
10. フォーラムURLの本文取得は `curl` / `wget` を優先し、取得失敗時はすぐに代替手段へ切り替える。
...
1. 入力が URL の場合は次の順で本文取得する。`curl -fsSL -A "Mozilla/5.0" "<URL>"` → 失敗時は `wget -qO- "<URL>"`。取得できた本文から事実情報（環境、手順、実際結果、期待結果）を抽出する。
...
7. URL本文を取得できない場合は、失敗理由を1行で記録し、投稿本文の貼り付けを依頼する。
8. 情報不足がある場合は、追加確認項目を短く列挙する。
