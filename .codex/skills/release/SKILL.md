...
その後、`.claude/skills/release/SKILL.md` の「引数なしで開始した場合」に定める項目を1問ずつ質問する。リリース対象は、リリース準備PRがマージされた後の `main` の先端に固定し、`main` へ直接コミットしない。前回タグがない場合は比較開始コミットを `BASE_REF` として確認する。全項目を確認してから開始前チェックへ進む。開始前チェックで未対応の `WIP` / `BLOCKED` や未 push コミットなどの保留条件が見つかった場合は、変更操作へ進まず停止する。
