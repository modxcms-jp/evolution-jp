...
その後、`.claude/skills/release/SKILL.md` の「引数なしで開始した場合」に定める項目を1問ずつ質問する。バージョン回答後は前回タグの候補をバージョン順で1つ提示し、「比較対象の前回リリースタグは `release-X.Y.ZJ` で合っていますか？」と確認する。回答が「いいえ」の場合だけ、別のタグまたは比較開始コミットを質問する。全項目を確認してから開始前チェックへ進む。
