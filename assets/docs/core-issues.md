...
## モジュール実行 (a=112) のシェル外表示

- **発見日**: 2026-07-07
- **発見元**: 管理画面frameset廃止プラン
- **ファイル**: `manager/processors/module/execute_module.processor.php`, `manager/frames/menu.php`
- **課題**: モジュールは独自の完全HTMLを出力するため、シェル（メニュー/ツリー保持）内にAJAXで埋め込めない。iframe は移行ルールで禁止のため、フルウィンドウ表示（data-no-ajax）としている。モジュール利用中はツリー・グローバルナビが見えない。
- **改善案**: モジュール出力をバッファリングして `<html>` タグの有無で断片/完全ページを判定し、断片ならシェル内に表示する方式を検討。
- **関連ロードマップ**: frame要素廃止（フォローアップ）
