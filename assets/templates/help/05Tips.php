...
<p>クラッシュしたサイトを復旧させるためにリストアを行なう場合は、この手順が有効です。データをインポートしてから上書きでシステム・拡張機能をアップデートすることにより、フレッシュな状態のシステム上にコンテンツデータを戻すことができるため、安全な状態で再スタートできる可能性が高くなります。
</p>

<h3>サイトを他ドメイン・他サーバに移動する</h3>
<p>
MODxのサイト移管は簡単です。インストーラを用いず、ファイル一式とデータベースの中身をそのまま移動させるだけです。移動後、管理画面にアクセスし、グローバル設定の「アセットディレクトリの物理パス」と「MODx稼働ディレクトリの物理パス」の値をリセットしてください。
</p>

<h3>データベースをMySQL4.0系からMySQL5系へ移行したい</h3>
<p>
基本的にはサイトのリストア作業に近いです。ファイル構成はそのままにしておいて、新しいデータベースでバックアップデータをインポートします。次に、MODxのインストーラのカスタムアップデートオプションを用いて、config.inc.phpを更新します。MODxのバージョンをアップデートしない場合でも、この作業は必ず行なってください。
</p>
...
<h3>検索窓のデザインをコントロール</h3>
<p>
AjaxSearchが出力する検索窓は、サイトによっては入力フィールド・検索ボタンなどの構成要素の並びが不自然に感じられることがあるでしょう。これは、ブロック要素であるfieldset・labelタグで各要素がwrapされているためです。<p>
<pre>
&lt;style type=&quot;text/css&quot;&gt;
&lt;!--
...
<p>
このようにスタイルを設定すると、表示に関する影響を抑えることができます。AjaxSearchはコードをhtmlソースに出力しませんので、細かいカスタマイズには<a href="http://www.google.com/search?hl=ja&q=firebug+%97v%91f%82%F0%92%B2%8D%B8" target="_blank">Firebug</a>などを用いるとよいでしょう。
</p>
<p>
...
<p>
上記のように記述すると、出力を2つのプレースホルダーに分割できます。検索窓を表示したいポイントに[+as.inputForm+]を、検索結果一覧を出力したい領域に[+as.results+]と記述してください。このパラメータを指定した場合、スニペットコールを記述したポイントには何も出力しません。同じ場所に[+as.inputForm+]を記述するとよいでしょう。</p>

<h3>検索結果一覧のリンクに付加されるクエリストリングを抑制する</h3>
<pre>
...
<h3>テンプレート単位でリッチテキストエディタの有効無効を設定したい</h3>
<p>
...
<p>
上記のようなプラグインを作り、システムイベントOnDocFormPrerenderをチェックします。上記の例ではID=7のテンプレートを設定しているリソースでリッチテキストエディタが無効になるように設定しています。
</p>

<h3>管理画面の文言をカスタマイズする</h3>
<p>
...
<p>
<a href="index.php?a=75">ユーザ管理</a>の対象ユーザのアカウント編集ページの「詳細」タブを開き、「管理画面ログイン開始ページ」としてトップページのリソースIDを指定します。こうすると、対象ユーザがログインした時、自動的にサイトのトップページにリダイレクトされます。QuickManagerを併用すれば、管理画面を使わずフロントエンドだけで基本的なコンテンツ管理ができます。
</p>

<h3>QuickManagerのインライン編集ボタンの使い方(1)</h3>
<p>
QuickManager1.4.1より、インライン編集機能が実装されました。これは旧MODx0.9系の頃に利用されていたQuickEditプラグインの機能に倣ったもので、content・pagetitle・longtitleなど、リソース変数ごと・テンプレート変数ごとに編集ボタンを表示します。「この領域を編集する」状態が分かりやすく、より直感的な編集操作を行なうことができます。QuickManagerプラグインの設定画面で「Inline template variable buttons」オプションをtrueに設定した上で、リソース変数・テンプレート変数を下記のように記述します(※例示です)。</p>
<pre>
[*<span style="color:red">#</span>content*]
[*<span style="color:red">#</span>テンプレート変数*]
</pre>
<p>
「#」を変数名の先頭に追記します。
</p>

<h3>QuickManagerのインライン編集ボタンの使い方(2)</h3>
<p>
Dittoなどのように複数のリソースを束ねて表示しているページで、個々のリソース単位で編集ボタンを表示できます。QuickManagerの「Inline edit buttons」オプションをtrueに設定した上で、
</p>
<pre>
&lt;!-- qm-edit [+id+] 'ここを編集' --&gt;
</pre>
...
<h3>QuickManagerのインライン編集ボタンの使い方(3)</h3>
<p>
...
<ul>
<li>/dir1/dir2/page.html … 設置ディレクトリ基準の絶対パス</li>
<li>dir1/dir2/page.html … 設置ディレクトリ基準の相対パス</li>
<li>http://example.com/dir1/dir2/page.html … URL</li>
</ul>
<p>
...
<p>
TinyMCEのようにパス形式を選択できる場合、どの形式がよいかは目的によって異なります。コンテンツとの整合性が正確で問題が起きにくいのは絶対パスやURL、コンテンツデータの可搬性に優れているのは相対パスです。たとえばJavaScriptやAjax技術による特殊な効果を盛り込みたい場合は絶対パスやURLが無難ですし、サブディレクトリでテスト構築したものを本番移行させるような構築フローを組む場合は相対パスが便利です(※<a href="http://www.google.com/search?hl=ja&q=html+base" target="_blank">baseタグ</a>の記述が必要です)。
</p>
<p>
