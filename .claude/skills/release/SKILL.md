...
続けて、次の質問を一度に提示する。既に回答済みの項目は再質問しない。

1. リリースするバージョンは何か（`X.Y.ZJ` 形式、例: `1.4.0J`）
2. リリース日を今日の日付にしてよいか。別の日付なら指定する
3. 現在の `main` の先端をリリース対象としてよいか。別のブランチ・コミットなら指定する
4. リリースノートの比較対象となる前回タグは何か。適切なタグがない場合は、比較開始に使うコミットを指定する
5. リリースノートは日本語のドラフトを生成し、ユーザー確認後に適用する流れでよいか
6. ZIP確認後の公開までこのセッションで進めるか、ドラフト作成・検証までで止めるか
