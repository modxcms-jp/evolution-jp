...
まず、ステップ6の手順1（GraphQL で unresolved スレッド一覧取得）を先に実行する。REST `/comments` には `isResolved` がないため、resolved 済みスレッドを分類対象から除外するために必要。
...
- /reviews本文（コメント概要）→ [修正方針]

### 対応を検討（ユーザー判断を仰ぐ）
- スレッド xxxx（先頭コメント概要）→ [対応した場合の影響と見送った場合のリスク]
- /reviews本文（コメント概要）→ [対応した場合の影響と見送った場合のリスク]

### 対応しない（理由付き）
- スレッド xxxx（先頭コメント概要）→ スタイルの好みのため見送り
- /reviews本文（コメント概要）→ スタイルの好みのため見送り
