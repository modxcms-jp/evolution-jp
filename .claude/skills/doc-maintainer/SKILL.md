...
1. まず「`/pr-sync <PR番号>` を実行するか？ はい・いいえ」で確認する
2. `はい` の場合のみ、`gh pr view <PR番号> --json body` で現在の PR 概要を取得する
3. `gh pr diff <PR番号>` または `git log <base>..HEAD --oneline` で実際の変更内容を確認する
4. 以下の観点で乖離を検出する:
...
5. 乖離箇所を列挙してユーザーに確認を求める
6. 確認後、`gh pr edit <PR番号> --body "..."` で更新する
