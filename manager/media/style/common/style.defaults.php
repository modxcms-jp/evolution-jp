<?php
if (!function_exists('apply_manager_style_defaults')) {
    function apply_manager_style_defaults(string $themePath, array $overrides = []): void
    {
        ...
        $config = array_merge([
            ...
            'iconSysInfo' => 1,
            'iconHelp' => 1,
            'manager_menu_height' => 86,
        ], $overrides);

        $configPath = "{$themePath}/config.php";
        if (is_file($configPath)) {
            $configFromFile = include $configPath;
            if (is_array($configFromFile)) {
                $config = array_merge($config, $configFromFile);
            }
        }

        include_once "{$themePath}/welcome.php";

        if ($config['manager_menu_height'] !== null) {
            $menuHeight = (int)$config['manager_menu_height'];
            if (!isset($modx->config['manager_menu_height']) || (int)$modx->config['manager_menu_height'] < $menuHeight) {
                $modx->config['manager_menu_height'] = (string)$menuHeight;
            }
        }

        if ($config['tab_your_info'] == 1) tabYourInfo();
        if ($config['tab_online'] == 1) tabOnlineUser();

        if ($config['iconResources'] == 1) iconResources();
        if ($config['iconNewDoc'] == 1) iconNewDoc();
        if ($config['iconSearch'] == 1) iconSearch();
        if ($config['iconMessage'] == 1) iconMessage();

        if ($config['iconElements'] == 1) iconElements();
        if ($config['iconFileManager'] == 1) iconFileManager();

        if ($config['iconSettings'] == 1) iconSettings();
        if ($config['iconEventLog'] == 1) iconEventLog();
        if ($config['iconSysInfo'] == 1) iconSysInfo();
        if ($config['iconHelp'] == 1) iconHelp();
        ...
        $_style['tree_folder'] = $tree_path . 'folder.png';
        $_style['tree_deletedfolder'] = $tree_path . 'deletedfolder.png';
        $_style['tree_folderopen'] = $tree_path . 'folderopen.png';
        ...
        $_style['icons_loading_doc_tree'] = $icon_path . 'information.png';
        $_style['icons_mail'] = $icon_path . 'email.png';
        ...
        $_style['icons_working'] = $icon_path . 'exclamation.png';
