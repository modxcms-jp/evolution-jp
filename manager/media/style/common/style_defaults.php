...
if (!function_exists('manager_style_prepare_welcome_options')) {
    function manager_style_prepare_welcome_options(array $optionNames = []): array
    {
...
if (!function_exists('manager_style_initialize_theme')) {
    function manager_style_initialize_theme(
        &$style,
        array $lang,
        ?array $welcomeOptions = null,
        int $menuHeight = 86
    ): void {
        $welcomeOptions = $welcomeOptions ?: manager_style_prepare_welcome_options();

        manager_style_set_default_welcome_options($welcomeOptions);
        manager_style_render_welcome_options($welcomeOptions);

        manager_style_set_default_menu_height($menuHeight);
        manager_style_set_defaults($style, $lang);
    }
}
