...
$_lang["modx_charset_message"] = '管理画面で使用したいエンコードを選択できます。設定変更後にconfig.inc.php内のエンコード指定も変更する必要があります。<br />※標準ではUTF-8固定です。選択肢を増やしたい場合はcharsets.phpを編集してください。';
$_lang["modx_charset_title"] = 'システムエンコード';
$_lang["chunk"] = 'チャンク';
...
$_lang["clean_uploaded_filename_title"] = 'アップロードファイルの名前の扱い';
...
$_lang["datepicker_offset_title"] = 'デートピッカーオフセット:';
$_lang["datepicker_offset_message"] = 'デートピッカー(カレンダー)で参照・選択できる過去年数(※日付情報は手入力で書き換えることもできます)';
$_lang["datetime_format_title"] = '日時の書式';
$_lang["datetime_format_message"] = '管理画面内で使用する日時の書式を選択してください。';
$_lang["default"] = '既定値';
$_lang["cache_default_message"] = 'リソース新規作成時のキャッシュ設定をデフォルトで有効にする場合は「はい」を選択してください。';
$_lang["cache_default_title"] = 'デフォルトでキャッシュを有効';
$_lang["auto_menuindex_message"] = '「はい」を選択すると、リソースを新規に作成する時にメニューオーダーの値が自動的に増分します。';
$_lang["auto_menuindex_title"] = 'メニューオーダー<br />自動増分';
$_lang["publish_default_message"] = '新しく作成するリソースをデフォルトで公開に設定する場合は「はい」を選択してください。';
$_lang["publish_default_title"] = '公開デフォルト';
$_lang["search_default_message"] = '新しく作成するリソースをデフォルトで検索対象とする場合は「はい」を選択してください。';
$_lang["search_default_title"] = '検索可能デフォルト';
$_lang["default_template_message"] = '新規に作成するリソースのデフォルトテンプレートを指定します。テンプレートはリソース単位で自由に選択できます。';
$_lang["default_template_title"] = 'デフォルトテンプレート';
$_lang["auto_template_logic_title"] = 'デフォルトテンプレート';
$_lang["auto_template_logic_message"] = 'リソース新規作成時のテンプレートの初期設定。';
...
$_lang["allow_duplicate_alias_message"] = '同じ階層内でエイリアス名の重複を許可します。この場合、エイリアス名をURL表現に用いると問題が発生する可能性があるため、ご注意ください。';
$_lang["allow_duplicate_alias_title"] = '重複エイリアスを許可';
...
$_lang["error_page_message"] = 'リソースが存在しない場合のエラーページ(404 ページ)として表示する、リソースのIDを入力してください。<br /><strong>※非公開またはウェブユーザ限定のリソースは指定できません。</strong>';
$_lang["error_page_title"] = 'エラーページ';
...
$_lang["friday"] = '金';
$_lang["friendly_alias_urls_message"] = 'フレンドリーURLを使用して、かつリソースがエイリアスを持つ場合、URL表現にエイリアスを用います。このオプションを「はい」に設定することで、フレンドリーURLのプレフィックスとサフィックスもエイリアスに適用されます。例えば、リソースのIDが1、エイリアスが「introduction」、プレフィックスが「」(空白)、サフィックスが「.html」の場合、このオプションを「はい」にすると、URLは「introduction.html」となります。 エイリアスを設定していない場合は、MODXが「1.html」というリンクを生成します。';
$_lang["friendly_alias_urls_title"] = 'エイリアスを使用';
$_lang["friendly_urls_message"] = 'MODX設置ディレクトリのsample.htaccessを.htaccessとしてリネームしてください。フレンドリーURL機能が動作しない場合は.htaccessをテキストエディタで開いて、コメントアウトされている「Options +FollowSymlinks」の記述を有効にしてみてください。';
$_lang["friendly_urls_title"] = 'フレンドリーURLを使用';
$_lang["friendly_url_prefix_message"] = 'フレンドリーURLのプレフィックスを指定してください。例えばプレフィックスを「page」に設定した場合、URLの /index.php?id=2 は、 /page2.htmlというフレンドリーURLに変換されます(サフィックスが .htmlに設定されていると仮定)。';
$_lang["friendly_url_prefix_title"] = 'プレフィックス';
$_lang["friendly_url_suffix_message"] = 'フレンドリーURLのサフィックスを指定してください。例えば「.html」と指定すると、全てのフレンドリーURLの末尾に .htmlを追加します。';
$_lang["friendly_url_suffix_title"] = 'サフィックス';
...
$_lang["number_of_logs_message"] = '<a href="index.php?a=13">管理操作ログ</a>閲覧時、1ページあたりに表示されるログエントリー数を入力してください。';
$_lang["number_of_logs_title"] = 'ログエントリー数';
$_lang["number_of_messages_message"] = 'メッセージ閲覧時、受信ボックスに表示されるメッセージ数を入力してください。';
$_lang["number_of_messages_title"] = 'メッセージ数';
$_lang["none"] = 'なし';
$_lang["number_of_results_message"] = '管理画面内の各機能の検索結果の一覧で、ページあたり何件表示するかを指定してください。';
$_lang["number_of_results_title"] = '結果の件数';
...
$_lang["use_browser_message"] = '「はい」を選択するとファイルブラウザを使用できます。ファイルブラウザは投稿画面から呼び出します。サーバー上に画像などをアップロードし、投稿画面に貼り付けることができます。この設定はimages・files・flash・mediaの4つのディレクトリを参照します。参照範囲が広い<a href="index.php?a=31">「ファイル管理」</a>と違い、ファイルブラウザは投稿画面との連動を前提としており、主にコンテンツに紐付くファイルのみを扱います。';
$_lang["use_browser_title"] = 'ファイルブラウザを使用';
...
$_lang["server_offset_time_message"] = 'サーバー設置場所とあなたの居場所の時差を選択してください。サーバーの現在時刻は<strong>[%s]</strong>です。オフセットを使用したサーバーの現在時刻は<strong>[%s]</strong>です。';
$_lang["server_offset_time_title"] = 'サーバーオフセットタイム';
...
$_lang["strip_image_paths_message"] = 'ファイルブラウザで選択したファイルのsrc属性を相対パスで投稿画面に渡します。この設定を「いいえ」にした場合は、httpから始まるフルURLになります。投稿画面で利用するRTEによっては、受け取った値をさらに処理することがあるため、ここでの設定は確実ではありません。<br />また、相対パスで運用する場合はフレンドリーURL設定を有効にすると画像やサイト内リンクなどを正しく参照できないことがあります。そのような場合は<a href="http://www.google.com/search?hl=ja&q=html+base+href" target="_blank">baseタグ</a>を利用してください。';
$_lang["strip_image_paths_title"] = '相対パスを渡す';
...
$_lang["showing"] = '表示中';
$_lang["signupemail_message_message"] = 'ユーザ管理画面でユーザーアカウントを作成した時に、ログイン名とパスワードを記載したメールを送信することができます。このメールの内容は変更できます<br /><strong>参考:</strong> 下記の差し込み文字列は送信時に自動的に置き換えられます。<br /><br />[(site_name)] - サイトの名前<br />[(emailsender)] - サイト管理者のメールアドレス<br />[(site_url)] - サイトのURL<br />[+username+] - ログイン名<br />[+password+] - パスワード<br />[+fullname+] - フルネーム';
$_lang["signupemail_message_title"] = 'サインアップメール';
$_lang["site"] = 'メイン';
$_lang["site_schedule"] = '公開状態の一覧';
$_lang["site_name_message"] = 'サイト名を入力してください。title要素などで [*pagetitle*] などと組み合わせて用います。<br />コンフィグ変数： [(site_name)]';
$_lang["site_name_title"] = 'サイト名';
$_lang["site_slogan_message"] = 'サイトの説明を入力してください。コンフィグ変数： [(site_slogan)]';
$_lang["site_slogan_title"] = 'サイトの説明';
$_lang["site_start_message"] = 'トップページとして使用するリソースのIDを入力してください。<br /><strong>※非公開のリソースは指定できません。</strong>';
$_lang["site_start_title"] = 'サイトスタート';
$_lang["site_status_message"] = 'このサイトをウェブ上に公開するには「オンライン」を選択してください。「メンテナンスモード」を選択すると、訪問者には「メンテナンス中ページ」または「メンテナンス中メッセージ」が表示されます。';
$_lang["site_status_title"] = '公開ステータス';
$_lang["site_unavailable_message_title"] = 'メンテナンス中メッセージ';
$_lang["site_unavailable_message_message"] = 'サイトがメンテナンスモード中、またはエラー発生時に表示するメッセージです。<br />コンフィグ変数：[(site_unavailable_message)]<br /><strong>※このメッセージはメンテナンス中ページが設定されていないときのみ表示されます。</strong>';
$_lang["site_unavailable_message_default"] = 'ただいま調整中です。しばらくお待ちください。';
$_lang["site_unavailable_page_message"] = 'メンテナンスモード設定時に表示するリソースのIDをここに入力してください。<br /><strong>※非公開またはウェブユーザー限定のリソースは指定できません。</strong>';
$_lang["site_unavailable_page_title"] = 'メンテナンス中ページ';
$_lang["snippet"] = 'スニペット';
...
$_lang["signupemail_message_default"] = "こんにちは [+username+] さん\n\n「[(site_name)] ( [(site_url)] )」の管理画面へのログイン情報について下記のとおりお知らせします。\n\nログイン名: [+username+]\nパスワード: [+password+]\n\n管理画面にログインすれば、パスワードを自由に変更できます。\n\n以上、よろしくお願いします。\nサイト管理者より";
$_lang["webpwdreminder_message_default"] = "こんにちは [+username+] さん\n\nパスワードを有効にするためには、下記のリンクをクリックしてください。\n\n[(site_url)]\n\n処理完了後、下記のパスワードをログインに使用できるようになります。\n\nパスワード:[+password+]\n\nこのメールに覚えが無いときは無視して下さい。\n\n以上、よろしくお願いします。\nサイト管理者より";
$_lang["websignupemail_message_default"] = "こんにちは [+username+] さん\n\n「[(site_name)] ( [(site_url)] )」へのログイン情報について下記のとおりお知らせします。\n\nログイン名: [+username+]\nパスワード: [+password+]\n\n [(site_name)]へログインすれば、自分のパスワードは自由に変更できます。\n\n以上、よろしくお願いします。\nサイト管理者より";
...
$_lang["unauthorized_page_title"] = '権限外告知のページ';
...
$_lang["upload_files_message"] = 'アップロードできる文書ファイルの拡張子を入力してください。カンマで区切って複数指定できます。';
$_lang["upload_files_title"] = 'アップロード可能な文書ファイル';
$_lang["upload_flash_message"] = 'アップロードできるフラッシュファイルの拡張子を入力してください。カンマで区切って複数指定できます。';
$_lang["upload_flash_title"] = 'アップロード可能なフラッシュ';
$_lang["upload_images_message"] = 'アップロードできる画像ファイルの拡張子を入力してください。カンマで区切って複数指定できます。';
$_lang["upload_images_title"] = 'アップロード可能な画像ファイル';
$_lang["upload_media_message"] = 'アップロードできるメディアファイルの拡張子を入力してください。カンマで区切って複数指定できます。';
$_lang["upload_media_title"] = 'アップロード可能なメディア';
...
$_lang["webpwdreminder_message_message"] = 'ウェブユーザーからパスワード変更の要求があった時に、MODXは新パスワードとアクティベーション方法を記載したメールを送信することができます。このメールの内容は変更できます。<br /><strong>参考:</strong> 下記の差し込み文字列は送信時に自動的に置き換えられます。<br /><br />[(site_name)] - サイトの名前<br />[(emailsender)] - サイト管理者のメールアドレス<br />[(site_url)] - サイトのURL<br />[+username+] - ログイン名<br />[+password+] - パスワード<br />[+fullname+] - フルネーム';
$_lang["webpwdreminder_message_title"] = 'パスワード変更<br />リクエストメール';
$_lang["websignupemail_message_message"] = 'ユーザー自らがウェブユーザーアカウントを作成した時に、MODXはログイン名とパスワードを記載したメールを送信することができます。このメールの内容は変更できます。<br /><strong>参考:</strong> 下記の差し込み文字列は送信時に自動的に置き換えられます。<br /><br />[(site_name)] - サイトの名前<br />[(emailsender)] - サイト管理者のメールアドレス<br />[(site_url)] - サイトのURL<br />[+username+] - ログイン名<br />[+password+] - パスワード<br />[+fullname+] - フルネーム';
$_lang["websignupemail_message_title"] = 'ウェブサインアップメール';
$_lang["wednesday"] = '水';
...
$_lang['output_filter_message'] = 'Output Filterを有効にします。Revolution互換設定の場合はカスタムモディファイアのプレフィックス文字列(phx:)が不要になります。';
...
$_lang["image_limit_width_title"] = '画像の横幅の最大値';
$_lang["image_limit_width_message"] = 'アップロードする画像の横幅を自動的に縮小します。空白にすると処理しません。';
