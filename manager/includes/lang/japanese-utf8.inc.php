...
$_lang["filemanager_path_message"] = '<a href="index.php?a=31">「ファイル管理」</a>で扱える範囲を決定します。コンテンツ編集に利用するファイルブラウザと違い、同じサーバ内で権限を持つ領域であればどのディレクトリでも管理でき、デフォルトではMODxをインストールしたディレクトリの物理パスを指します。IISなど一部のサーバでは物理パスが適切に得られないことがあるためご注意ください。';
...
$_lang["mgrlog_actionid"] = 'ActionID';
...
$_lang["rb_base_dir_message"] = 'アセットディレクトリ(通常はassetsディレクトリ)の物理パスを入力してください。通常この設定は自動的に生成されますが、IISなど一部のサーバでは正常に検出されないことがあります。この場合は手入力で入力してください。';
$_lang["rb_base_dir_title"] = 'アセットディレクトリの<br />物理パス ';
$_lang["rb_base_url_message"] = 'アセットディレクトリ(通常はassetsディレクトリ)のパスを入力してください。MODx設置ディレクトリを基準とした相対パスを記述します。通常この設定は自動的に生成されますが、IISなど一部のサーバではインストール時の検出が正常にできていないことがあります。';
$_lang["rb_base_url_title"] = 'アセットディレクトリの<br />相対パス';
...
$_lang["showing"] = '表示中';
$_lang["signupemail_message"] = 'ユーザ管理画面でユーザーアカウントを作成した時に、ログイン名とパスワードを記載したメールを送信することができます。このメールの内容は変更できます<br /><strong>参考:</strong> 下記の差し込み文字列は送信時に自動的に置き換えられます。<br /><br />[+sname+] - サイトの名前, <br />[+saddr+] - サイト管理者のメールアドレス, <br />[+surl+] - サイトのURL, <br />[+uid+] - ユーザーのログイン名, <br />[+pwd+] - ユーザーのパスワード, <br />[+ufn+] - ユーザーのフルネーム. ';
...
$_lang["table_hoverinfo"] = 'ここで取得したSQLファイルを用いて、サイトをリストアできます。phpMyAdminなどを用いてSQLファイルをインポートします。「DROP文を生成」にチェックを入れておくと、インポート時にテーブルを作り直すため、より確実にサイトを再現できます。<br /><strong style="color:red;">【注意1】</strong>SQLファイルのサイズが100MB以上に及ぶと、リストア(インポート)に失敗しやすくなるためご注意ください。特に_event_logテーブルと_manager_logテーブルが肥大の原因になっていることが多いです。「データサイズ」のリンクをクリックしてログをリセットするか、これら2つのテーブルをバックアップ対象から外すとよいでしょう。<br /><strong style="color:red;">【注意2】</strong> データをリストアする時、リストア元とリストア先のMODxのバージョンが違う場合は注意が必要です。リストアすると、拡張機能のバージョンも元の状態に戻ります。問題を回避するには、MODxのバージョンを揃えてリストアするか、リストア後にもう一度MODxをインストーラを用いて最新状態にアップデートするとよいでしょう。';
