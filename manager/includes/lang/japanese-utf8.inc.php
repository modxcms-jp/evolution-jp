...
$_lang["about_msg"] = '<p>MODX(モドエックスまたはモッドエックス)は <a href="https://ja.wikipedia.org/wiki/MODx" target="_blank">PHP アプリケーション フレームワーク・コンテンツ管理システム</a>です。GPL に基づいて使用が許諾されます。</p>';
...
$_lang["automatic_optimize_table_message"] = 'ログのローテーション処理を行う際にテーブルの最適化(OPTIMIZE TABLE)も同時に実行します。';
$_lang["automatic_optimize_table_title"] = 'テーブルの自動最適化';
$_lang["backup"] = 'バックアップ';
...
$_lang["configcheck_php_gdzip_msg"] = 'このサーバでは<a href="https://www.google.com/search?q=gd+%22php.ini%22&lr=lang_ja" target="_blank">GDライブラリ</a>が読み込まれてません。<a href="https://www.google.com/cse?cx=007286147079563201032%3Aigbcdgg0jyo&ie=UTF-8&q=CAPTCHA&sa.x=26&sa.y=8&siteurl=http:%2F%2Fforum.modx.jp%2F" target="_blank">ログイン時の画像認証機能(CAPTCHA)</a>など画像まわりの処理に制限があります。';
...
$_lang["credits_shouts_msg"] = '<p>MODX is managed and maintained at <a href="https://www.modx.com/company/" target="_blank">modx.com</a>.</p>';
...
$_lang["eventlog_msg"] = 'システムログはJSONLines形式で保存されたエラー・警告・情報メッセージを表示します。';
$_lang["eventlog_viewer"] = 'システムログ';
$_lang["system_log"] = 'システムログ';
$_lang["system_log_msg"] = 'システムログはJSONLines形式で保存されたエラー・警告・情報メッセージを表示します。';
$_lang["latest_log"] = '最新ログ';
$_lang["latest_log_summary"] = '最新100件のログファイルを横断して表示しています。';
$_lang["recent_30_days"] = '直近30日';
$_lang["month_suffix"] = '月';
$_lang["keyword"] = 'キーワード';
$_lang["failed_delete"] = '削除に失敗しました。';
...
$_lang["export_site_maxtime_message"] = 'エクスポート処理の制限時間を秒数で指定してください(<a href="https://www.google.com/search?q=php+set_time_limit&hl=ja&lr=lang_ja" target="_blank">PHPの設定</a>を上書きします)。0を入力すると無制限に設定できますが、通常は制限時間を設定するようおすすめします。';
...
$_lang["folder"] = 'コンテナ';
$_lang["forgot_password_email_fine_print"] = '注意：上記のURLは30分間有効です。';
...
$_lang["guid"] = 'GUID';
$_lang["help"] = 'ヘルプ';
$_lang["help_msg"] = '<p><a href="https://forum.modx.jp/" target="_blank">MODX日本公式フォーラム</a>でサポートを得ることができます。 また、逐次執筆が進められている<a href="https://rtfm.modx.com/display/Evo1/Home" target="_blank">MODXドキュメントとガイド（英語）</a>があり、MODX のほぼすべてに関して記されています。</p>';
...
$_lang["import_site_maxtime_message"] = 'MODXがサイトのインポート処理に使用できる最大秒数を指定します。（<a href="https://www.google.com/search?q=php+set_time_limit&hl=ja&lr=lang_ja" target="_blank">PHPの指定処理秒数</a>を上書きします）「0」を指定すると処理時間を制限しませんが、想定外の処理時間を要することもあるためご注意ください。';
...
$_lang["manager_safe_mode_button_disable"] = 'セーフモードを解除';
$_lang["manager_safe_mode_button_enable"] = 'セーフモードで表示';
$_lang["manager_safe_mode_nav_disable"] = 'セーフモード解除';
$_lang["manager_safe_mode_nav_enable"] = 'セーフモード';
...
$_lang["module_msg"] = 'モジュール名には日本語を用いることができます。';
...
$_lang["none"] = 'なし';
$_lang["noresults_message"] = '管理画面内のリスト表示において、ページあたり何件表示するかを指定してください。';
...
$_lang["php_version_check"] = 'MODX EvolutionはPHPバージョン5.3.0またはそれ以上で動作します。インストールされているPHPをアップグレードしてください。';
$_lang["plugin"] = 'プラグイン';
...
$_lang["recommend_setting_change_description"] = 'リファラチェック設定が無効になっているため、意図しない管理画面操作による<a href="https://www.google.com/search?hl=ja&q=csrf" target="_blank">CSRF攻撃 (Cross Site Request Forgery attack)</a>を受ける恐れがあります。グローバル設定をご確認ください。ただしこの設定により管理画面にアクセスできなくなる可能性があります。その場合はパソコン側のセキュリティソフトの設定をチェックするか、他のパソコンからのアクセスを試してみてください。';
...
$_lang["snippet_msg"] = '<p>スニペット名には日本語を用いることができます。</p>';
...
$_lang["table_hoverinfo"] = 'ここで取得したSQLファイルを用いて、サイトをリストアできます。「DROP文を生成」にチェックを入れておくと、インポート時にテーブルを作り直すため、より確実にサイトを再現できます。<br /><strong style="color:red;">【注意1】</strong>SQLファイルのサイズが大き過ぎるとリストア(インポート)に失敗しやすくなるためご注意ください。特に_event_logテーブルと_manager_logテーブルが肥大の原因になっていることが多いです。「データサイズ」のリンクをクリックしてログをリセットするか、これら2つのテーブルをバックアップ対象から外すとよいでしょう。それでもまだ大きい場合は<a href="https://www.google.com/search?hl=ja&q=BigDump" target="_blank">BigDump</a>などのツールを試してみてください。<br /><strong style="color:red;">【注意2】</strong> データをリストアする時、リストア元とリストア先のMODXのバージョンが違う場合は注意が必要です。リストアすると、拡張機能のバージョンも元の状態に戻ります。問題を回避するには、MODXのバージョンを揃えてリストアするか、リストア後にもう一度MODXをインストーラを用いて最新状態にアップデートするとよいでしょう。';
...
$_lang["which_editor_message"] = 'デフォルトで使用するリッチテキストエディターを選択してください(※<a href="https://modx.com/extras/category/richtexteditors?product=evolution" target="_blank">MODXのダウンロードページ</a>には追加でインストールできるリッチテキストエディターがあります)。';
...
$_lang['setting_disable_cache_at_login'] = 'ログイン時のキャッシュ';
$_lang['setting_disable_cache_at_login_desc'] = 'ログイン時はキャッシュを無効にし常に動的出力します。';
$_lang['setting_convert_datauri_to_file'] = 'Data URI の自動ファイル化';
$_lang['setting_convert_datauri_to_file_desc'] = 'リソース保存時に Data URI スキーム (data:image/png;base64,...) を検出し、自動的にファイルに変換します。画像編集後の Data URI を含むコンテンツのファイルサイズを削減できます。保存先: content/images/datauri/{リソースID}/';
$_lang['duplicate_title_string'] = 'コピー - [+title+]';
$_lang["confirm_delete_category"] = 'このカテゴリー名を削除してもよろしいですか?';
$_lang["site_url_title"] = 'サイトのURL';
$_lang["site_url_message"] = '[(site_url)] - 例：<b>[+MODX_SITE_URL+]</b><br />サイトのURL。空白にすると自動的に生成しますが、サーバの設定によってはクライアントが意図的な値で生成することが可能です。他のサイトのURLに改ざんされる可能性があるため、ここで値を固定することをおすすめします。';
...
$_lang["mutate_settings.dynamic.php8"] = 'エラー発生時に[(emailsender)]([+emailsender+])に通知します。エラーの内容の詳細は<a href="index.php?a=114">システムログ</a>を確認してください。';
...
$_lang["base_url_message"] = '[(base_url)] - 例：<b>[+MODX_BASE_URL+]</b><br />MODXの設置ディレクトリ。空白にすると動的に生成しますが、ここで値を固定することをおすすめします。';

$_lang["configcheck_del_actionphp"] = 'セキュリティ警告: action.phpが存在します';
$_lang["configcheck_del_actionphp_msg"] = 'Captcha画像の表示に用いるaction.phpは1.0.6J-r4以降で不要になりました。セキュリティ上の問題があるため、削除するか、<a href="https://modx.jp/news/140.html" target="_blank">問題がFIXされたaction.php</a>を使用してください。';
...
$_lang['enable_bindings_message'] = '<a href="https://www.google.com/cse?cx=007286147079563201032%3Aigbcdgg0jyo&q=Bindings" target="_blank">@Bindings機能</a>を有効にします。この機能は、投稿画面上の入力フィールド(テンプレート変数)に任意のコマンドを記述し、実行するものです。PHP文の実行などが可能なため、複数メンバーでサイトを運用する場合、当機能の運用には注意が必要です。';
...
$_lang['modxmailer_log_title'] = 'MODxMailerのシステムログ';
$_lang['modxmailer_log_message'] = 'MODxMailerでメール送信する際にPOSTデータと送信元・送信先情報をシステムログに記録します。';
...
$_lang["draft_data_publishdate"] = '採用日時';
