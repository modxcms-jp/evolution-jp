...
$_lang["captcha_words_default"] = 'maguro,toro,tako,ika,hotate,awabi,kazunoko,ebi,kani,uni,iwashi,aji,saba,tamago,negitoro,tekka,hamachi,sanma';
...
$_lang["htmlsnippet_management_msg"] = '<h3 style="font-weight:bold;">チャンクの管理</h3>チャンクは、パーツ感覚で利用できる「生の」HTMLコードです。 <strong>{{チャンク名}}</strong> と記述するだけで、リソースやテンプレートから手軽に呼び出すことができます。';
...
$_lang["plugin_management_msg"] = '<h3 style="font-weight:bold;">プラグインの管理</h3>プラグインはシステム寄りの拡張機能で、実行するためには任意のシステムイベントに関連付ける必要があります。スニペットほど直感的ではありませんが、管理画面のカスタマイズもできるなど応用幅が広いです。詳しくは<a href="index.php?a=9">ヘルプ</a>をご覧ください。';
...
$_lang["search_results_returned_msg"] = '<strong>%s</strong>件のデータが見つかりました。多くの検索結果が表示される場合は、検索範囲を更に狭くしてください。左側二つのカラムでは、リソースツリーでリソースを探したり、リソースを表示したりすることができます。右側二つのカラムでは、 リソースの削除状態および公開状態が各々表示されます。<p />';
...
$_lang["tmplvars_management_msg"] = '<h3 style="font-weight:bold;">テンプレート変数の管理</h3>一般的なCMSでいうところのカスタムフィールドに相当するもので、MODXはこの機能が非常に柔軟・強力です。「タイトル」や「内容」などの既定の項目に加え、投稿画面上に自由に項目を追加できます。テンプレート内の任意の場所に <strong>[*テンプレート変数名*]</strong> と記述すると値を出力できます。詳しくは<a href="index.php?a=9">ヘルプ</a>を参照してください。';
