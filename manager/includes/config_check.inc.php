...
if (!is_writable($modx->config['rb_base_dir'] . 'images')) {
    $warnings[] = 'configcheck_images';
}

if (!is_dir($modx->config['rb_base_dir'])) {
    $warnings[] = 'configcheck_rb_base_dir';
}
if (!is_dir($modx->config['filemanager_path'])) {
        ...
        switch ($warning) {
            ...
            case 'configcheck_configinc';
                ...
                if (!$_SESSION["mgrConfigCheck"]) {
                    $modx->logEvent(0, 2, $output, $_lang[$warning]);
                }
                break;
            case 'configcheck_installer':
                $output = $_lang['configcheck_installer_msg'];
                if (!$_SESSION["mgrConfigCheck"]) {
                    $modx->logEvent(0, 3, $output, $_lang[$warning]);
                }
                break;
            ...
            case 'configcheck_danger_ajaxsearch':
                ...
                if (!isset($_SESSION["mgrConfigCheck"])) {
                    $modx->logEvent(0, 3, $output, $_lang['configcheck_danger_ajaxsearch']);
                }
                break;
