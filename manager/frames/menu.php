                ...
                <ul id="nav">
                    ...
                    if (!empty($sitemenu)) {
                        echo $modx->parseText($tplActive,
                            array('id' => '1', 'name' => $_lang['site'], 'menuitem' => join("\n", $sitemenu)));
                    }
                    if (!empty($elementmenu)) {
                        echo $modx->parseText($tpl,
                            array('id' => '2', 'name' => $_lang['elements'], 'menuitem' => join("\n", $elementmenu)));
                    }
                    if (!empty($modulemenu)) {
                        echo $modx->parseText($tpl,
                            array('id' => '3', 'name' => $_lang['modules'], 'menuitem' => join("\n", $modulemenu)));
                    }
                    if (!empty($securitymenu)) {
                        echo $modx->parseText($tpl,
                            array('id' => '4', 'name' => $_lang['users'], 'menuitem' => join("\n", $securitymenu)));
                    }
                    if (!empty($usermenu)) {
                        echo $modx->parseText($tpl,
                            array('id' => '7', 'name' => $_lang['user'], 'menuitem' => join("\n", $usermenu)));
                    }
                    if (!empty($toolsmenu)) {
                        echo $modx->parseText($tpl,
                            array('id' => '5', 'name' => $_lang['tools'], 'menuitem' => join("\n", $toolsmenu)));
                    }
                    if (!empty($reportsmenu)) {
                        echo $modx->parseText($tpl,
                            array('id' => '6', 'name' => $_lang['reports'], 'menuitem' => join("\n", $reportsmenu)));
                    }
                    ?>
                </ul>
            </div>
