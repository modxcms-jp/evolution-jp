        ...
        <div class="tab-pane" id="sysinfoPane">
            <div class="tab-page" id="tabServer">
                ...
                <div class="sectionBody" id="lyr2">
                    <table border="0" cellspacing="2" cellpadding="2">
                        <?php echo render_tr(lang('modx_version'),config('settings_version'));?>
                        <?php echo render_tr(lang('release_date'),$modx_release_date);?>
                        <?php echo render_tr('システム更新日時',evo()->toDateFormat(config('lastInstallTime')));?>
                        <?php echo render_tr('phpInfo()','<a href="#" onclick="viewPHPInfo();return false;">' . lang('view') . '</a>');?>
                        <?php echo render_tr(lang('udperms_title'),(config('use_udperms')==1 ? lang('enabled') : lang('disabled')));?>
                        <?php echo render_tr(lang('servertime'),strftime('%H:%M:%S', time()));?>
                        <?php echo render_tr(lang('localtime'),strftime('%H:%M:%S', time()+config('server_offset_time')));?>
                        <?php echo render_tr(lang('serveroffset'),config('server_offset_time')/(60*60) . ' h');?>
                        <?php echo render_tr(lang('database_name'),db()->dbname);?>
                        <?php echo render_tr(lang('database_server'),db()->hostname);?>
                        <?php echo render_tr(lang('database_version'),db()->getVersion());?>
                        <?php
                        $rs = db()->query("show variables like 'character_set_database'");
                        $charset = db()->getRow($rs, 'num');
                        echo render_tr(lang('database_charset'),$charset[1]);
                        ?>
                        <?php
                        ...
                        ?>
                        <?php echo render_tr(lang('table_prefix'),db()->table_prefix);?>
                        <?php echo render_tr(lang('cfg_base_path'),MODX_BASE_PATH);?>
                        <?php echo render_tr(lang('cfg_base_url'),MODX_BASE_URL);?>
                        <?php echo render_tr(lang('cfg_manager_url'),MODX_MANAGER_URL);?>
                        <?php echo render_tr(lang('cfg_manager_path'),MODX_MANAGER_PATH);?>
                        <?php echo render_tr(lang('cfg_site_url'),MODX_SITE_URL);?>
                    </table>
                </div>
            </div>
            ...
            <div class="tab-page" id="tabActivity">
                ...
                <div class="sectionBody" id="lyr1">
                    <?php echo lang('sysinfo_activity_message'); ?><p>
                        <style type="text/css">
                    ...
                    <table class="grid">
                        ...
                        $rs = db()->select($field,evo()->getFullTableName('site_content'),'deleted=0','editedon DESC',20);
                ...
                <div class="sectionBody" id="lyr4">
                    <table class="grid">
                        ...
                        <tr>
                            ...
                            <td width="120" align="right"><b><?php echo lang('database_table_effectivesize'); ?></b></td>
                        ...
                        for ($i = 0; $i < $limit; $i++) {
                            ...
                            <tr bgcolor="<?php echo $bgcolor; ?>" title="<?php echo $log_status['Comment']; ?>" style="cursor:default">
                                ...
                                <td dir="ltr" align="right"><?php echo evo()->nicesize($log_status['Data_length']+$log_status['Data_free']); ?></td>
                                <?php

                                if(evo()->hasPermission('settings')) {
                                    echo  "<td align='right'>".($log_status['Data_free']>0 ? "<a href='index.php?a=54&mode=$action&t=".$log_status['Name']."' title='".lang('optimize_table')."' ><span dir='ltr'>".evo()->nicesize($log_status['Data_free'])."</span></a>" : "-")."</td>";
                                }
                                else {
                                ...
                                ?>
                                <td dir='ltr' align="right"><?php echo evo()->nicesize($log_status['Data_length']-$log_status['Data_free']); ?></td>
                                <td dir='ltr' align="right"><?php echo evo()->nicesize($log_status['Index_length']); ?></td>
                                <td dir='ltr' align="right"><?php echo evo()->nicesize($log_status['Index_length']+$log_status['Data_length']+$log_status['Data_free']); ?></td>
                            </tr>
                            <?php
                        ...
                        <tr bgcolor="#e0e0e0">
                            ...
                            <td colspan="3">&nbsp;</td>
                            <td dir='ltr' align="right" valign="top"><?php echo $totaloverhead>0 ? "<b style='color:#990033'>".evo()->nicesize($totaloverhead)."</b><br />(".number_format($totaloverhead)." B)" : "-"; ?></td>
                            <td colspan="2">&nbsp;</td>
                            <td dir='ltr' align="right" valign="top"><?php echo "<b>".evo()->nicesize($total)."</b><br />(".number_format($total)." B)"; ?></td>
                        </tr>
                        </tbody>
                    </table>
                    <?php
            ...
            <div class="tab-page" id="tabOnlineUsers">
                ...
                <div class="sectionBody" id="lyr5">
                    ...
                    <?php
                    $html = lang('onlineusers_message').'<b>'.strftime('%H:%M:%S', time()+config('server_offset_time')).'</b>):<br /><br />
				<table class="grid">
				<thead>
					<tr>
