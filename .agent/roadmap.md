...
- ExecPlan: `なし`
- メモ/判断ログ: 2026-05-03 に PR #423 レビューで指摘。バイパスモードは数十 KB のメモリで動作することを目標としており、Logger や autoload を持ち込む設計と根本的に相容れない。ログハンドリングは割り切りとして対象外とする方針。着手するとしてもバイパスモードの設計目標を見直す議論が先行する。
...
- 背景/課題: `frontend_log_shutdown_fatal` は index.php:241 で登録され、`postProcess` は `prepareResponse()` 内（line 629）で後から登録される。PHP は shutdown ハンドラを登録順に実行するため、postProcess 内で例外が発生した時点では既存ハンドラの制御フロー下にある。postProcess 内の Throwable は即座にログして対応する設計へ転換。
- 到達条件（Definition of Done）:
