...
- 到達条件（Definition of Done）:
  ...
  - 既存`document.parser.subparser.trait.php`を廃止する(ファイル削除・`use`文・`require_once`の除去)。ただし`loadExtension('subparser')`の互換シムと、末尾で定義されている`@deprecated SubParser`クラスの後方互換(新ファイルへの移設)は維持する
