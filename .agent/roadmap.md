...
最終更新: 2026-07-12 (AIランタイム運用設計の棚卸しタスクを追加し、共通ポリシー文書を新設)
...
- 非対象（やらないこと）: 全機能一括置換
- 依存関係: マイグレーション機構
- ExecPlan: `.agent/plans/2026-07-08-pdo-migration-dbapi-refactor.md`
- メモ/判断ログ: 影響範囲が大きいため段階移行を前提。フェーズ1=内部PDO化（API完全互換）、フェーズ2=プレースホルダ移行、フェーズ3=PostgreSQL/SQLite対応、と3段階に分割。今回のExecPlanはフェーズ1のみを扱う。
