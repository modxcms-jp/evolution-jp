...
最終更新: 2026-05-04 (静的ページキャッシュ経路の cache_status 識別 完了)
...
- 依存関係: fatal/OOM 調査ログ拡張
- ExecPlan: `なし`
- メモ/判断ログ: 2026-05-03 に PR #423 レビューで指摘。`Logger::pushEvent('static_cache.hit')` を exit 前に追加して recent_events で識別する方式が候補。2026-05-04 に `get_static_pages()` の `if ($content)` ブロック先頭（`parseDocumentSource()` 呼び出し前）に `Logger::pushEvent('static_cache.hit')` を追加して実装完了。これにより、静的キャッシュ経路で fatal が発生した際に `recent_events` で他の `unknown` 経路（フルページキャッシュバイパスなど）と区別できる。
