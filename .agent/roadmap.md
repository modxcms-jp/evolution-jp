...
- 到達条件（Definition of Done）:
  ...
  - 既存`document.parser.subparser.trait.php`を廃止する(ファイル削除・`use`文・`require_once`の除去)。ただし`loadExtension('subparser')`の互換シムは維持する
  - メソッド棚卸し(grep)による分割前後の完全一致確認、`docker compose exec app php evo help`によるフルブート確認、主要画面のブラウザスモークテストで検証する
...
- メモ/判断ログ: 2026-07-14 (yamamoto/Claude): public メソッドが manager/actions・プラグイン・モジュール等から広く直接呼ばれ、約100個のpublicプロパティを全メソッド群が共有するため、別クラス抽出より同一オブジェクト上でのトレイト分割を採用。着手順は独立性の高い`tv`から始め、本丸の`tag-parse`/`request-response`を最後に回す方針(詳細はExecPlanのDecision Log)。
