...
- 背景/課題: `frontend_log_shutdown_fatal` は index.php:241 で登録され、`postProcess` は `prepareResponse()` 内（line 633）で後から登録される。PHP は shutdown ハンドラを登録順に実行するため、postProcess 内で例外が発生した時点では既存ハンドラの制御フロー下にある。postProcess 内の Throwable は即座にログして対応する設計へ転換。
- 到達条件（Definition of Done）:
