...
- [ ] (2026-04-28) 未捕捉 Throwable / fatal error の捕捉
...
- **決定**: 通常の PHP warning handler だけでなく、管理画面アクションの未捕捉 `Throwable` と shutdown 時の fatal error をシステムログに記録する
- **理由**:
...
- **実装方針**:
  ...
  - shutdown handler は `E_ERROR`, `E_PARSE`, `E_CORE_ERROR`, `E_COMPILE_ERROR` などの fatal 系のみを対象にし、通常 warning と重複記録しない
...
7. 外部連携の拡張ポイントを固定する（Sentry実装はしない）。
