...
- [ ] (2026-04-26) システムログ画面の改修（ファイル一覧・表示）
...
- 2026-04-26: ホスト側PHPには `mysqli` 拡張がなく、`php evo help` はブートストラップ時点で停止した。Dockerコンテナ側では `mysqli/mysqlnd` が有効。DB接続失敗は `.env` の `DB_HOST=db-mysql8` がComposeネットワーク上で解決できないことが原因で、`DB_HOST=db` に変更すると `php evo help` とCMSブートストラップ込みの `logEvent()` 検証が成功した。
...
**UI構成**（ASCII art）:
    ...
    [ログファイル一覧]           [ログ内容]
    ┌─────────────────┐       ┌─────────────────────┐
    │ system-2026-04-26.log│  →  │ [Level: All ▼][🔍]  │
    │ 2.5MB  123 lines    │       │ ┌─────────────────┐│
    │ system-2026-04-25.log│       │ │ JSON表示        ││
    │ 1.8MB  89 lines     │       │ │ (整形済み)      ││
    │ ...                 │       │ └─────────────────┘│
    │ [削除] [ダウンロード]│       │ [全選択] [保存]      │
    └─────────────────┘       └─────────────────────┘
...
**バックエンド処理**（Ajax API）:
    ...
    // ログ内容取得
    GET /manager/ajax.php?action=log_read&file=system-2026-04-26.log&level=error
    → {"logs": [{...}, {...}]}

    // ログ削除
    POST /manager/ajax.php?action=log_delete&file=system-2026-04-26.log
    → {"success": true}
