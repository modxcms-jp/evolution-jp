...
- [ ] (2026-07-07) M6: 自走表示確認 — curl検証は完了（54アクション200・断片応答・リダイレクト・未ログイン検出・a=84/93疎通）。ユーザーのブラウザ確認で発見された5件の不具合を修正済み。ブラウザでの最終確認（保存フロー・履歴・未保存警告・RTL）が残り。2026-08-15: Playwrightで保存・履歴・未保存警告を実確認し、RTLはDOM属性を適用した表示でツリー右配置を確認。利用可能な日本語・英語の言語ファイルはいずれもLTRのため、サーバー設定によるRTL実設定確認は未実施。
- [x] (2026-07-11) マネージャ内最後のframeset(mcpukファイルブラウザの7フレーム構成)を解消。別ExecPlan(.agent/plans/2026-07-11-file-browser-modal-rewrite.md)でゼロから作り直し、旧mcpukディレクトリを削除。`grep -rn "frameset" manager/`がヒットしないことを確認し、マネージャ内のframe要素は本タスクを含め全廃完了。
