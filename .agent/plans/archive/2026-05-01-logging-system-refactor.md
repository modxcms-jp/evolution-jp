...
### 2026-05-01: ドキュメント文脈を Logger の共通収集に寄せる

- **決定**: `document_identifier` / `document_method` / `decoded_request_uri` は `Logger::collectContext()` で取得可能な場合に自動収集する
- **理由**:
  - `request.url` は自動収集されている一方、`DocumentParser::phpError()` 経由の warning では文書単位の調査に必要な `documentIdentifier` が欠けていた
  - 各呼び出し元で個別に積むと漏れや重複が発生しやすく、SSOT が崩れる
  - フロントエンド warning / error の解析では request 単位だけでなく document 単位の相関が必要
- **実装方針**:
  - `evo()` が利用可能な場合のみ収集し、未初期化フェーズでは空値を無理に出さない
  - 既存の `active_plugin` / `current_snippet` と同じ層で共通コンテキストとして記録する
  - 個別エラー処理側では追加取得せず、ロガー側に集約する
...
3. `timestamp` の定義を見直し、必要に応じて `request_started_at` のような別フィールドを追加する
4. 文脈収集とファイル出力の責務分割を検討し、将来の拡張時に影響範囲を局所化する
...
- **ドキュメント文脈の共通収集**: `document_identifier` / `document_method` / `decoded_request_uri` を取得可能な範囲で自動記録
...
**ファイル配置**: `manager/includes/cli/commands/log-{tail,search}.php`
...
- インストーラの退避ログを `temp/logs/install/YYYY/MM/` 配下へ移動し、DB 接続後も install ログのまま残す
...
6. CLI コマンド群を追加する。
   編集対象ファイル: `manager/includes/cli/commands/log-tail.php`, `manager/includes/cli/commands/log-search.php`
   実行コマンド: `php -l manager/includes/cli/commands/log-tail.php manager/includes/cli/commands/log-search.php`
...
6. 既存コードから `evo()->logEvent(101, 3, 'Document not found', 'DocumentParser')` を実行する。
   期待観測結果: 新ログへ `source=DocumentParser`、`level=error`、相対パス化された caller / trace を含むエントリが記録される。
...
1. `git diff -- manager/includes/logger.class.php manager/includes/traits/document.parser.subparser.trait.php manager/actions/report/eventlog.dynamic.php manager/includes/cli/commands/log-tail.php manager/includes/cli/commands/log-search.php install/sql/create_tables.sql` で差分を確認する。
2. 直近作業を破棄する場合は `git restore --source=HEAD -- manager/includes/logger.class.php manager/includes/traits/document.parser.subparser.trait.php manager/actions/report/eventlog.dynamic.php manager/includes/cli/commands/log-tail.php manager/includes/cli/commands/log-search.php install/sql/create_tables.sql` を実行する。
...
### ログエントリの例

    {
      ...
      "context": {
        ...
        "trace": [
          {"file": "manager/includes/document.class.php", "line": 100, "function": "editDocument", "class": "DocumentParser"}
        ]
      }
    }
