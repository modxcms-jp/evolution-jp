...
- [x] (2026-08-15) M6: 自走表示確認 — curl検証は完了（54アクション200・断片応答・リダイレクト・未ログイン検出・a=84/93疎通）。Playwrightで保存・履歴・未保存警告を実確認し、一時RTL言語設定による実サーバー出力（`html[dir="rtl"]`、ツリー右配置、frame系要素0件）も確認済み。
