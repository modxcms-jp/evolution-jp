...
- 2026-05-07: `/pr-sync` コマンドを doc-maintainer スキルに追加し、ワークフローへの組み込みも同 PR で完了。当初 `CLAUDE.md` の「レビュー時」セクションへ追記する予定だったが、後続作業でワークフロー全体を `AGENTS.md` へ移動したため、組み込み先は `AGENTS.md` の「タスク管理ワークフロー → レビュー時」に変更。
...
- `AGENTS.md` — ワークフロー定義（PR作成・レビュー時の手順）※当初 `CLAUDE.md` を対象としていたが、SSOT整備でワークフローを `AGENTS.md` へ移動した
...
PR 作成後や追加コミット後に `/pr-sync` を実行するステップを `AGENTS.md` の「タスク管理ワークフロー → レビュー時」に追記。`.claude/skills/doc-maintainer/SKILL.md` にも使い方ガイダンスを添える。
...
**1-1. `AGENTS.md` の「タスク管理ワークフロー → レビュー時」セクションを編集**
...
※当初 `CLAUDE.md` を対象としていたが、SSOT整備でワークフローを `AGENTS.md` へ移動したため対象変更。

**1-2. `.claude/skills/doc-maintainer/SKILL.md` に使い方ガイダンスを追記**
...
**確認:** `AGENTS.md` の「タスク管理ワークフロー → レビュー時」手順に `/pr-sync` の記載があること。
...
- `AGENTS.md` の「タスク管理ワークフロー → レビュー時」に `/pr-sync` が含まれている
...
- 参考メモ: 解消済み問題の記録（メモファイルは作成せず、このExecPlanの Decision Log に集約）
