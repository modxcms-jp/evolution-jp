...
- ユーザー確認後にのみ、`.agent/roadmap.md` の `Status: DONE` と `完了日` を更新する
- ユーザー確認後にのみ、完了した ExecPlan は `.agent/plans/archive/` へ移動する
- アーカイブ時はファイル名先頭の日付を完了日（`YYYY-MM-DD`）へ更新する
- ロードマップに `ExecPlan:` を記載している場合は、移動後のパスへ更新し、`.agent/roadmap.md` の `最終更新` も同期する
