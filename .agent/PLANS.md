...
8. **スキル成長ループ**: `docker compose exec <app-service> php evo skill:complete --plan=<plan-slug>` をアプリコンテナ内で実行し、run の learning/pruning/proposal を生成する（`skill:init` が未実行の場合は先に `php evo skill:init --plan=<plan-slug> --skill=<skill-name>` を実行する）
