...
8. **スキル成長ループ**: `docker compose exec <app-service> php evo skill:complete --plan=<plan_id> --run-id=<run_id> --strict` をアプリコンテナに対してホストから実行し、run の整合検証と learning/pruning/proposal を生成する（`skill:init` が未実行の場合は先に `docker compose exec <app-service> php evo skill:init --plan=<plan_id> --skill=<skill-name>`（`<skill-name>` は例: `project-worker`） を実行する。`run_id` は `docker compose exec <app-service> php evo skill:status --plan=<plan_id>` で確認できる）
