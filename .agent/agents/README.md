...
- `assets/docs/ai-runtime-governance.md`: ランタイム分担と推論運用の正本
...
- 役割設計とランタイム設計は分離し、Codex / Claude / Copilot の使い分けは `assets/docs/ai-runtime-governance.md` を参照する。
...
- 複数 AI や sub-agent の利用可否は「役割を分ける価値があるか」で決め、ランタイム名から逆算しない。
...
レビューでは、ユーザーが「レビュー」「PR確認」「差分確認」「レビュー指摘に対応」などを依頼した場合に `review-agent` スキルを入口にする。
