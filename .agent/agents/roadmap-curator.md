...
5. ExecPlan の実装と検証が完了したら完了を告げてユーザー確認を求め、確認後に `.agent/roadmap.md` の `Status` / `完了日` / `最終更新` を更新する。
6. 完了確認後に、完了した ExecPlan は `.agent/plans/archive/` へ移動し、ファイル名先頭の日付を完了日へ更新し、ロードマップの `ExecPlan:` パスも同期する。
7. 依存順の大幅変更や複数タスク削除は、ユーザー確認を必要とする。
