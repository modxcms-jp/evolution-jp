...
$_lang["access_permissions_links_tab"] = 'どのユーザグループがドキュメントグループにアクセス(例 編集、サブドキュメントの作成)できるかを指定します。ドキュメントグループをユーザグループに関連付けるには、ドロップダウンメニューよりドキュメントグループを選択して「グループの追加」をクリックしてください。ドキュメントグループとの関連付けを削除するには、「削除 ->」をクリックしてください。この作業を行うことにより、すぐに関連付けが解除されます。';
...
$_lang["allow_mgr_access_message"] = 'このオプションはマネージャへのアクセスをユーザに許すか、拒否するかを選択するものです。<strong>注意: このオプションを「いいえ」に設定すると、ユーザは上記の「マネージャログイン開始ページ」か、空白ならサイトスタートのページへリダイレクトされます。</strong>';
...
$_lang["captcha_words_message"] = '「CAPTCHAコードの使用」を「はい」にした場合に使用されるCAPTCHAワードリストを入力してください。CAPTCHAワードはカンマで区切ってください。このフィールドには、255文字まで入力することができます。';
...
$_lang["charset_message"] = 'マネージャで使用したいエンコードを選択してください。日本語も含め、ほとんどの言語ではデフォルト設定のUTF-8が利用できます。EUCを選択する場合はEUC版のlangファイルをフォーラムより入手しインストールする必要があります。ShiftJISは十分に検証されていません。';
...
$_lang["configcheck_cache_msg"] = 'キャッシュディレクトリにキャッシュファイルを保存できませんでした。MODxは問題なく動作しますが、キャッシュが効かないためレスポンスが低下します。この問題を解消するには、 /_cache/ ディレクトリを書き込み可能にしてください。';
$_lang["configcheck_configinc"] = '設定ファイルがまだ書き込み可能になっています';
$_lang["configcheck_configinc_msg"] = '悪意ある人はあなたのサイトや関連するあらゆるものを壊すことができてしまいます。<strong>本当です！</strong> 設定ファイル (/manager/includes/config.inc.php)のパーミッションを404などに設定し、書込み不可にしてください。';
...
$_lang["configcheck_installer_msg"] = 'install/ ディレクトリにMODxインストーラが残されています。悪意の攻撃を受ける可能性がありますので、早急にこのディレクトリを削除することをお勧めします。';
...
$_lang["configcheck_notok"] = '<span style="font-weight:bold;color:red;">設定に問題があります。</span>';
$_lang["configcheck_ok"] = '確認OK - 警告はありません。';
$_lang["configcheck_register_globals"] = '現在の環境では register_globals が ON に設定されており危険です。';
$_lang["configcheck_register_globals_msg"] = 'クロスサイトスクリプティング攻撃(略してXSS)を受けやすい脆弱性があります。XSS攻撃の対象はMODx本体であるとは限らず、サードパーティによって開発されるスニペットやプラグインが狙われることもあります。register_globals設定を特に必要としない場合は、.htaccessまたはphp.iniに一行を追記し、この設定を無効にしてください。';
...
$_lang["database_overhead"] = '<strong style=\'color:#990033\'>注:</strong>オーバーヘッドはMySQLが確保しているけれど使用はしていない領域のことです。オーバーヘッドの数字をクリックすると、この領域を開放します。';
...
$_lang["defaultcache_message"] = '全ての新しいドキュメントをデフォルトでキャッシュ可能にする場合は「はい」を選択してください。';
$_lang["defaultcache_title"] = 'キャッシュデフォルト';
$_lang["defaultmenuindex_message"] = 'メニューインデックスを自動的に増分する機能を有効にするには「はい」を選択してください。';
$_lang["defaultmenuindex_title"] = 'メニューインデックス自動増分';
$_lang["defaultpublish_message"] = '全ての新しいドキュメントをデフォルトで公開する場合は「はい」を選択してください。';
$_lang["defaultpublish_title"] = '公開デフォルト';
$_lang["defaultsearch_message"] = '全ての新しいドキュメントをデフォルトで検索可能にする場合は「はい」を選択してください。';
...
$_lang["document_alias_help"] = 'このドキュメントのエイリアスを指定することができます。次のようにドキュメントにアクセスすることができます:\n\nhttp://yourserver/エイリアス\n\nエイリアスはフレンドリーURLを使用する場合のみ動作します。テンプレート記述：[*alias*]';
$_lang["document_content"] = '内容 ( テンプレート記述：[*content*] )';
$_lang["document_description"] = '説明';
$_lang["document_description_help"] = 'ドキュメントに関する任意の説明をここに入力することができます。テンプレート記述：[*description*]';
$_lang["document_long_title_help"] = 'ドキュメントの長いタイトルを入力してください。これはサーチエンジンに対して効果があります。ドキュメントの詳細な情報を記述することができます。テンプレート記述：[*longtitle*]';
$_lang["document_metatag_help"] = 'このドキュメントに付加したいMETAタグやキーワードを選択してください。CTRLキーをクリックと同時に押すと複数のタグやキーワードを選択できます。選択元とするMETAタグ・キーワードの作成・管理は「リソース」→<a href="index.php?a=81" target="_self">「METAタグとキーワードの管理」</a>で行ないます。';
...
$_lang["document_opt_menu_title_help"] = 'メニュータイトルは、メニュースニペットやモジュールで使用できるドキュメントの短いタイトルです。テンプレート記述：[*menutitle*]';
$_lang["document_opt_published"] = '公開する';
$_lang["document_opt_published_help"] = '保存後、すぐにドキュメントを公開する場合はチェックしてください。';
$_lang["document_opt_richtext"] = 'リッチテキスト編集';
...
$_lang["document_summary_help"] = 'ドキュメントの要約を簡潔に入力してください。テンプレート記述：[*introtext*]';
$_lang["document_title"] = 'タイトル';
$_lang["document_title_help"] = 'ドキュメントの名称/タイトルを入力してください。バックスラッシュは使用できません。(テンプレート記述：[*pagetitle*])';
...
$_lang["duplicate_alias_message"] = '「はい」を選択すると重複するエイリアスを保存できるようになります。<br />※エイリアスパスによる階層表現に対応してなかった頃の名残で、基本的にMODxではエイリアス名の重複を認めていません。「index.html」などエイリアス名の重複が発生すると思われる運用をする場合は、この項目を「はい」にしてください。';
...
$_lang["error"] = 'エラー';
$_lang["errorpage_message"] = 'ドキュメントが存在しない場合のエラーページ(404ページ)として表示するドキュメントのIDを入力してください。<strong>注意: 既に存在するドキュメントのIDを入力してください。ドキュメントは公開されている必要があります!</strong>';
$_lang["errorpage_title"] = 'エラーページ:';
$_lang["error_sending_email"] = 'メール送信中にエラー';
$_lang["eventlog"] = 'システムログ';
$_lang["eventlog_msg"] = 'システムログはMODxが作成する告知・警告・エラーの各メッセージを表示するものです。「元」列にはメッセージを発したマネージャの部分を表示します。';
$_lang["eventlog_viewer"] = 'システムログ';
...
$_lang["export_site_numberdocs"] = '<strong>%s 個のエクスポート対象ドキュメントが見つかりました...</strong><p/>';
...
$_lang["files_upload_inhibited_msg"] = '<strong>アップロード機能が使用できません。</strong> - アップロードがサポートされているか、PHPがディレクトリに書込権を有しているかを確認してください。<br />';
...
$_lang["friendlyurls_message"] = 'SEOに有効な（フレンドリーな）URLの利用を設定します。この機能はMODxがApache上で動作している時のみ有効です。この機能を動作させるために .htaccessファイルを用意する必要があります。詳細は同梱の .htaccessファイルをご覧ください。';
...
$_lang["lock_settings_msg"] = '%s が設定を編集中です。他のユーザが終了するまでしばらくお待ちください。';
...
$_lang["login_message"] = 'MODxへようこそ。ユーザ名とパスワードは大文字・小文字を区別します。';
$_lang["logout"] = 'ログアウト';
...
$_lang["mgr_login_start_message"] = 'ユーザがマネージャにログインしたとき、リダイレクトしたいドキュメントのIDを入力します。<strong>注意: 入力するIDは、既に存在するドキュメントIDであり、ドキュメントは公開されていてこのユーザがアクセス可能である必要があります!</strong>';
...
$_lang["no"] = 'いいえ';
$_lang["nologentries_message"] = '管理操作ログ閲覧時、1ページあたりに表示されるログエントリ数を入力してください。';
...
$_lang["security"] = 'ユーザ管理';
$_lang["select_date"] = '日付を選択';
$_lang["send"] = '送信';
$_lang["servertime"] = 'サーバ時刻';
$_lang["serveroffset"] = 'サーバオフセット';
$_lang["serveroffset_message"] = 'サーバ設置場所とあなたの居場所の時差を選択してください。サーバの現在時刻は<strong>[%s]</strong>です。オフセットを使用したサーバの現在時刻は<strong>[%s]</strong>です。';
...
$_lang["tree_show_protected_message"] = '「いいえ」にセットするとドキュメントツリーの中で保護されたページは表示されなくなります（親ドキュメントが保護されている場合はサブドキュメントも表示されません）。"No" is the legacy setting for MODx.';
...
$_lang["udperms_message"] = 'アクセス権限設定を有効にすると、ユーザの編集権限をユーザグループごと・ドキュメントグループごとに割り当てることができます。まず<a href="index.php?a=40" target="_self">マネージャへのアクセス許可</a>を開き、ユーザグループとドキュメントグループを作ってください。次に、それぞれのユーザグループとドキュメントグループを関連付けてください。最後に、個別のユーザをユーザグループに、個別のドキュメントをドキュメントグループに割り当ててください。この設定を「はい」にした場合、全てのドキュメントを編集する権限を持つのは初期状態では管理者のみとなります。';
...
$_lang["upload_maxsize_message"] = 'ファイルマネージャを通じてアップロード可能な最大サイズを入力してください。サイズはバイト単位で入力します。<strong>注意: 大きなサイズのファイルのアップロードには長時間を要するかもしれません。</strong>';
...
$_lang["user_edit_self_msg"] = 'ユーザ情報を変更した場合は、変更を反映させるためにログアウトして再度ログインしてください。<br />新しいパスワードの作成をチェックした場合、新しいパスワードがメール送信されます。</b>';
