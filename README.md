...
Evolution CMS JP Edition は、ツリー型のページ管理と軽快なレスポンスを特徴とするオープンソース CMS「MODX Evolution」を日本での利用向けに調整したディストリビューションです。公式サイトの [modx.jp](https://modx.jp/) と連携しながら、ローカライズ済みのセットアップや技術ドキュメントを提供します。CMS 全体の特徴は公式チュートリアル「[Evolution CMS で作るウェブサイト](https://modx.jp/docs/tutorial/quick.html)」にもまとまっており、階層ツリーでページを整理しながら高速に公開できるサイト構築基盤として紹介されています。
...
- **ローカライズ済みの運用環境**: 管理画面やセットアップウィザードはローカライズされ、UTF-8 ベースの言語ファイルやサンプルデータが同梱されています。
...
- **多層キャッシュ機構**: ファイルキャッシュや静的 HTML など複数のレイヤーを活用して公開レスポンスを最適化します。詳細は `documents/cache-mechanism.md` にまとめています。
- **Data URI 最適化**: 長大な Data URI を自動的にファイルへ変換するユーティリティを同梱し、メディア管理を効率化します。詳細は `documents/datauri-conversion.md` を参照してください。
...
| `install/` | セットアップスクリプトと多言語リソースです。新規インストールやアップデートで使用します。|
| `manager/` | 管理画面とコア PHP クラスの実装です。`DocumentParser` や拡張 API が含まれます。|
| `temp/` | キャッシュ、バックアップ、静的 HTML などを格納する一時ディレクトリです。|

## 動作要件

- Web サーバー (Apache / Nginx など)
- PHP 7.0 以上（7.4 以降を推奨）
- MySQL 互換データベース (MySQL 5.x / MariaDB など)
